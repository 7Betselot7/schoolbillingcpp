            ...
            for (int i = 9; i <= 12; ++i)
                {
                    for (int j = 0; j < ACS; ++j)
                    {
                        stringstream updateDatesSQL;
                        updateDatesSQL << "UPDATE Semester_Info SET ";

                        bool needsCommaDate = false;
                        if (!idate[j].empty()) {
                            if (needsCommaDate) updateDatesSQL << ", ";
                            updateDatesSQL << "INITIAL_DATE = '" << idate[j] << "'";
                            needsCommaDate = true;
                        }
                        if (!edate[j].empty()) {
                            if (needsCommaDate) updateDatesSQL << ", ";
                            updateDatesSQL << "END_DATE = '" << edate[j] << "'";
                            needsCommaDate = true;
                        }
                        if (!pdate[j].empty()) {
                            if (needsCommaDate) updateDatesSQL << ", ";
                            updateDatesSQL << "PUNISH_DATE = '" << pdate[j] << "'";
                            needsCommaDate = true;
                        }

                        // Add the WHERE clause to update specific rows
                        updateDatesSQL << " WHERE GRADE = " << i << " AND NO = " << (j + 1);

                        char* errorMessage;
                        int exit = sqlite3_exec(db, updateDatesSQL.str().c_str(), nullptr, 0, &errorMessage);
                        if (exit != SQLITE_OK) {
                            cerr << "Error Updating Dates Data: " << errorMessage << endl;
                            sqlite3_free(errorMessage);
                        } else {
                            cout << "Dates updated successfully for grade " << i << " and NO " << (j + 1) << endl;
                        }
                    }
                }
...
if (answer == 1) {
    for (int i = 9; i <= 12; ++i) {
        ...
        for (int j = 0; j < ACS; ++j) {
            stringstream updateGradeSQL;
            updateGradeSQL << "UPDATE Semester_Info SET ";

            bool needsComma = false;  // Reset needsComma for each new SQL statement

            if (!regFee.empty()) {
                if (needsComma) updateGradeSQL << ", ";
                updateGradeSQL << "REG_FEE = " << regFee;
                needsComma = true;
            }

            if (!amount.empty()) {
                if (needsComma) updateGradeSQL << ", ";
                updateGradeSQL << "AMOUNT = " << amount;
                needsComma = true;
            }

            if (!punishmentAmount.empty()) {
                if (needsComma) updateGradeSQL << ", ";
                updateGradeSQL << "PUNISHMENT_AMOUNT = " << punishmentAmount;
                needsComma = true;
            }

            // Add the WHERE clause to update specific rows
            updateGradeSQL << " WHERE GRADE = " << i << " AND NO = " << (j + 1);

            char* errorMessage;
            int exit = sqlite3_exec(db, updateGradeSQL.str().c_str(), nullptr, 0, &errorMessage);
            if (exit != SQLITE_OK) {
                cerr << "Error Updating Grade Data: " << errorMessage << endl;
                sqlite3_free(errorMessage);
            } else {
                cout << "Data updated successfully for grade " << i << " and NO " << (j + 1) << endl;
            }
        }
    }
}
